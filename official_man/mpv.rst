...
（以下按键绑定可在控制台中打开一个选择器，通过输入所需项目部分的内容、单击所需项目或使用按键绑定导航，从项目列表中进行选择： ``Down`` 和 ``Ctrl+n`` 向下滚动， ``Up`` 和 ``Ctrl+p`` 向上滚动， ``PGDWN`` 和 ``Ctrl+f`` 向下滚动一页， ``PGUP`` 和 ``Ctrl+b`` 向上滚动一页。）

在轨道选择器中，再次选择当前轨道会禁用它。
