...
``loadfile <url> [<flags> [<index> [<options>]]]``
    ...
    <play>
        若当前无内容播放，则开始播放（始终从添加的文件开始播放，即使在执行此命令前播放列表为非空状态）。

    可组合使用多个标志，例如： ``append+play``

    默认情况下，即使播放列表先前为空， ``append`` 、 ``insert-next`` 和 ``insert-at`` 也不会立即开始播放。添加  ``play`` 标志可强制开始播放。

    以下值已过时，它们是 mpv 0.42 版本之前强制播放开始的旧方法（在添加 ``play`` 标志之前）。
    ...
    第三个参数是插入索引，仅用于 ``insert-at`` 操作。与这些操作一起使用时，新项目将被插入播放列表中的索引位置，如果索引小于 0 或大于播放列表的大小，则会被追加到末尾。所有其他操作都将忽略此参数。此参数自mpv0.38.0版本起加入。
...
``loadlist <url> [<flags> [<index>]]``
    ...
    <play>
        如果当前没有正在播放的内容，则开始播放（始终从添加的播放列表开始播放，即使在执行此命令之前内部播放列表并非空状态）。
    ...
    第三个参数是插入索引，仅用于 ``insert-at`` 操作。与这些操作一起使用时，新的播放列表将插入到内部播放列表中的索引位置，如果索引小于 0 或大于内部播放列表的大小，则附加到末尾。所有其他操作都将忽略此参数。此参数自mpv0.38.0版本起加入。
...
``deinterlace-active``
    如果 mpv 的去隔行扫描滤波器处于活动状态，则返回 ``yes``/true 。这只是使用 ``--deinterlace=auto`` 参数时的猜测，可能不够准确。请注意，它不会检测任何通过 ``--vf`` 手动插入的隔行扫描滤波器。
...
``libplacebo-version``
    宏 ``PL_VERSION`` 的内容。这是一个字符串，通过发布版本号或Git哈希值来标识构建版本。

``subrandr-version``
    ``sbr_library_version()`` 的返回值，以字符串形式呈现，格式为 ``<major>.<minor>.<patch>`` ，表示运行时 subrandr 库的发布版本号。若 mpv 未启用 subrandr 编译，则此属性不可用。
