...
g-l
    选择一个要搜索的字幕行。目前，这要求在 ``PATH`` 中有 ``ffmpeg``，或在 Windows 下与 mpv 位于同一文件夹中
...
.. warning::
    ...
    目前，你可以在设置文件中以正常的方式扩展 ``pseudo-gui`` profile。这已经过时。在未来的mpv版本中，该行为可能会变更，不要应用你的额外设置，和/或使用一个不同的配置预设名称。
