...
``--flatten-editions=<yes|no>``
    某些容器格式（例如包含多个节目的 HLS 或 MPEG-TS）会提供多个节目或版本组。默认情况下，mpv 会遵循该格式，将轨道分组为不同版本，并过滤轨道列表，仅显示属于当前所选版本的轨道。

    将此选项设置为 ``yes`` 将忽略文件的节目结构。不会创建任何版本，所有节目的所有轨道都将作为平铺列表显示。请注意，根据文件的不同，来自不同节目的轨道可能彼此完全无关。

``--show-dependent-tracks=<yes|no>``
    在轨道列表中显示依赖轨道（默认： no）。依赖轨道包含无法独立解码的编码数据。例如，构成瓦片式 HEIF 图像的瓦片子流、IAMF 音频元素的原始编码层，或 LCEVC 组中的增强流。默认情况下这些轨道被隐藏，因为显示它们会使轨道列表充斥着无法单独进行有效选择的条目。
...
``--playlist=<filename>``
    ...
    默认情况下，mpv 不会播放来自被视为不安全的播放列表中的 URL。如果你信任该播放列表文件，可以使用 ``--load-unsafe-playlists`` 选项禁用所有安全检查。由于播放列表可能包含其他播放列表条目，建议仅对播放列表本身应用此选项，而非其条目，具体使用方式如下：
...
``--ytdl=<yes|no>``
    ...
    ``all_formats=<yes|no>``
        如果 "yes"，将尝试添加所有由youtube-dl报告的格式（默认： yes）。每种格式都作为一个单独的轨道被添加。此外，它们被延迟加载，且实际上只有在选择轨道时才会打开（这应该能使加载时间和没有这个设置选项时一样短）。
...
``--framedrop=<mode>``
    ...
    <vo>
        ...
        这是推荐的模式，也是默认的模式。
    <decoder>
        旧的、基于解码器的丢帧模式。这会告知解码器跳过某些帧（除非这些帧对于解码后续帧是必要的）。此方法可能对运行缓慢的系统有所帮助，但可能会导致画面卡顿、无法观看，甚至导致显示完全冻结。
...
``--hwdec=<api1,api2,...|no|auto|auto-copy>``
    ...
    .. admonition:: 选择哪种方式？
        ...
        - 如果想测试所有可用的硬件解码方式，传递 ``--hwdec=auto-unsafe --hwdec-codecs=all`` ，查看终端的输出。
...
``--image-subs-hdr-peak=<sdr|video|video-static|video-dynamic|10-10000>``
    控制 HDR 视频中图像字幕的漫反射白点亮度（单位：cd/m²（尼特））（默认值： 1000）。 ``sdr`` 对应标准 SDR 白点的 203 cd/m²， ``video`` 采用所有视频元数据（包括峰值检测）， ``video-dynamic`` 仅采用按场景划分的视频元数据， ``video-static`` 仅采用静态视频元数据。
    （ ``--vo=gpu-next`` 独占）
...
``--sub-margin-y-offset=<size>``
    在字幕边距中添加额外的垂直偏移量，单位为缩放像素。该值将叠加在 ``--sub-margin-y`` 之上。

    此选项旨在实现运行时的动态边距调整（例如通过 OSC 等脚本避免字幕与 UI 重叠）。若需持续性设置，请改用 ``--sub-margin-y`` 。
...
``--sub-justify=<auto|left|center|right>``
    控制多行字幕的对齐方式，而不考虑其对齐位置（默认： ``auto`` ，按照 ``--sub-align-x`` 的定义进行对齐）。
...
``--keep-open=<yes|no|always>``
    ...
    如果播放列表中有下一个文件，或者播放列表是循环的， ``yes`` 不会暂停。近似地，这将在播放器正常退出时暂停，但在实际中，有一些极端的情况不是这样的（例如， ``mpv --keep-open file.mkv /dev/null`` 将正常播放file.mkv，然后无法打开 ``/dev/null`` ，然后退出）。
...
``--drag-and-drop=<no|auto|replace|append|insert-next>``
    控制默认的内置拖放行为（ ``--input-builtin-drag-and-drop`` ）。
    ``auto`` 将遵循 ``dropped-files`` 属性中的 ``action`` 值。 ``replace`` 、 ``append`` 和 ``insert-next`` 分别始终强制替换、追加到以及插入到播放列表的下一位置。 ``no`` 禁用所有拖放行为。
...
``--geometry=<[W[xH]][+-x+-y][/WS]>`` , ``--geometry=<x:y>``
    ...
    .. note::
        ...
        几何尺寸和位置是在窗口初始化时应用的。后续的自动调整大小操作（通过 ``--auto-window-resize`` ）会重新应用 ``--geometry`` 指定的尺寸，但不会应用位置，除非启用了 ``--force-window-position`` 。在播放过程中更新 ``--geometry`` 时，无论 ``--force-window-position`` 的设置如何，都会应用该位置。
...
``--force-render``
    不论窗口的可见性，强制mpv始终渲染帧。目前只影响 X11 和 Wayland 和 macvk 视频输出驱动，因为只有它们具有这种优化（即其它的输出都是不考虑可见性进行渲染）。

``--force-window-position=<yes|no>``
    控制在因视频流大小变化而自动调整窗口大小后（参见 ``--auto-window-resize`` ），窗口是否会移动回初始位置。禁用时，窗口将保留当前位置，仅调整大小以适应新的视频尺寸。启用时，窗口将重置为由 ``--geometry`` 设定的初始位置，若未指定几何参数，则重置为屏幕中心。目前仅影响 Windows、X11、macvk 和 SDL VOs。（默认： no）
...
``--stop-screensaver=<yes|no|always>``
    ...
    在 Wayland，这取决于合成器是否支持空闲禁用协议。对于不支持该协议的合成器，您可以通过用户脚本定期调用 ``xdg-screensaver reset`` 来禁用屏幕保护程序。
...
``--demuxer-hysteresis-secs=<seconds>``
    ...
    默认值是0秒，这将禁用缓存迟滞。一个10秒的值可能对大多数情况都有效。

``--demuxer-hysteresis-bytes=<bytesize>``
    与 ``--demuxer-hysteresis-secs`` 相同，但将滞后量以向前缓冲数据的字节数（而非秒数）指定。一旦达到解复用器限制，解复用器将不再进行向前缓冲，直到向前缓冲的数据量降至该值。

    此选项适用于比特率可变或未知的流，在这些情况下，基于字节的阈值比基于时间的阈值更有意义。它也可以与 ``--demuxer-hysteresis-secs`` 结合使用。在这种情况下，当任一阈值达到时，缓冲都会恢复。

    默认值为 0，这将禁用基于字节的缓存迟滞。
...
``--directory-filter-types=<video,audio,image,archive,playlist>``
    打开目录时要过滤的媒体文件类型。若要将所有文件添加到播放列表中，请使用 ``--directory-filter-types-clr`` 清空列表。（默认： ``video,audio,image,archive,playlist`` ）

    这是一个字符串列表选项。详见 `列表选项`_

``--autocreate-playlist=<no|filter|same>``
    打开本地的常规文件时，就像打开父目录一样，自动创建播放列表。
...
``--input-builtin-drag-and-drop=<yes|no>``
    启用内置的拖放功能（默认： yes）。将其设置为 no 将禁用内置的拖放处理功能。
...
``--input-ipc-server=<filename>``
    ...
    在 Linux 和 Unix 系统上，如果路径的首字符是 ``@`` ，则系统会将其内部替换为空字符；在 Linux 系统中，该字符代表一个抽象套接字地址，但在其他 UNIX 平台上可能导致未定义的行为。否则，给定的路径即为常规文件系统路径。在 Windows 系统上，由于使用命名管道，因此该路径指向管道命名空间（ ``\\.\pipe\<name>`` ）。如果缺少 ``\\.\pipe\`` 前缀，mpv会在创建pipe前自动添加，所以 ``--input-ipc-server=/tmp/mpv-socket`` 和 ``--input-ipc-server=\\.\pipe\tmp\mpv-socket`` 对于Windows上的IPC是等效的。
...
``--osd-margin-y-offset=<size>``
    在 OSD 边距中添加额外的垂直偏移量，单位为缩放像素。该值将叠加在 ``--osd-margin-y`` 设置之上。

    此选项旨在实现运行时的动态边距调整（例如通过 OSC 等脚本避免 OSD 与 UI 重叠）。若需持续性设置，请改用 ``--osd-margin-y`` 。
...
``--screenshot-template=<template>``
    ...
    ``%{prop[:fallback text]}``
        ...
        .. note::
            ...
            自 mpv 0.42.0 版本起，该功能仅支持 C11 标准中定义的 ``strftime`` 格式指定符，或用于 UNIX 时间戳的 ``%s`` 扩展。其他格式指定符均不被支持。
...
``--interpolation``
    ...
    .. warning:: 这需要将 ``--video-sync`` 选项设置为 ``display-`` 模式之一，否则它将被默默禁用。
...
``--d3d11-output-format=<auto|rgba8|bgra8|rgb10_a2|rgba16f>``
    ...
    .. note::
        ...
        对于 ``--vo=gpu-next`` ， ``rgba16f`` 可启用scRGB输出。此外，还可以使用 ``--target-trc=scrgb`` 来指定 scRGB 输出。
...
``--wayland-edge-pixels-pointer=<value>``
    定义边缘边框的大小（默认： 16），以便使用鼠标或数位板在wayland上下文中启动client调整大小的events。仅当合成器中没有服务器端装饰时，此选项才能激活。

``--wayland-edge-pixels-touch=<value>``
    定义边缘边框的大小（默认： 32），以便使用鼠标在wayland上下文中启动client调整大小的events。仅当合成器中没有服务器端装饰时，此选项才能激活。
...
``--wayland-session=<string>``
    设置窗口恢复所用的 Wayland 会话名称（默认： ``unset`` ）。若未设置此选项或将其设为空字符串，则会禁用会话管理。
...
``--glsl-shader=<file>``, ``--glsl-shaders=<file-list>``
    ...
    每一段元数据，连同它后面的非元数据行，都定义了单一的块。目前有三种类型的块，HOOKs和TEXTUREs和PARAMs。
    ...
    PARAM <name> (required)
        开始一个参数块，用于定义可调的着色器参数。这些参数在整个着色器文件中是全局的，文件中的所有钩子都可以引用它们，无论声明顺序如何。

    TYPE [ENUM] <DEFINE | <type>> (required)
        参数类型。支持的类型为 ``float`` 和 ``int`` 。

        特殊类型 ``DEFINE`` 会生成一个预处理器定义，可在 ``#if`` 指令中使用。

        如果使用了 ``ENUM`` 限定符，则类型必须是 ``DEFINE`` 或 ``int`` 。带 ``ENUM`` 限定符的参数不接受默认值，而是通过换行符分隔列出所有可能的枚举值。这些值被分配从 0 开始、递增 1 的整数值。每个枚举项也会作为预处理器定义输出，并在 RPN 表达式中可用。``MINIMUM`` 和 ``MAXIMUM`` 将被忽略。
    ...
    参数的初始/默认值是参数头之后的第一行非元数据行。

    .. note::
        ``vo=gpu`` 仅支持 ``vo=gpu-next`` 中可用参数特性的子集。有关 ``vo=gpu-next`` 中支持的 PARAM 特性的更多详细信息，请参阅 libplacebo 文档。值得注意的是，``uint`` 、 ``DYNAMIC`` 和 ``CONSTANT`` 类型不可用。
    ...
    WIDTH <szexpr>, HEIGHT <szexpr>
        指定本次处理结果的纹理尺寸。 ``szexpr`` 指的是RPN（反向波尔符号）中的表达式，使用运算符+ - * / > < ! = %，浮点字数，以及对现有纹理（如MAIN.width或CHROMA.height）、OUTPUT、可调节的参数或NATIVE_CROPPED（经过pan-and-scan, video-align-x/y, video-pan-x/y等裁剪的输入纹理的尺寸，可能还有预缩放）的引用。默认情况下，这些被分别设置为HOOKED.w和HOOKED.h。
...
``--target-trc=<value>``
    ...
    s-log2
        索尼S-Log2曲线
    scrgb
        sRGB，扩展线性光传输。同时支持HDR和广色域内容。除非系统报告了显示器的原色，否则输出色域默认为BT.709。您也可以使用 ``--target-gamut`` 手动指定更宽的色域。
        （ ``--vo=gpu-next`` 独占）
...
``--target-gamut=<value>``
    限制显示的色域。您可以使用此选项输出例如 DCIP3-in-BT.2020 等色彩。将 ``--target-prim`` 设为包含色彩空间的基色（数值将被编码到基色中），将 ``--target-gamut`` 设为要限制色彩的色域。支持指定命名的原色集（与 ``--target-prim`` 参数的值相同），或通过 8 个以逗号分隔的 CIE xy 色度值来指定自定义原色，格式为 ``Rx,Ry,Gx,Gy,Bx,By,Wx,Wy`` （红、绿、蓝、白点）。所有值必须在 0.0-1.0 范围内。使用 ``--target-gamut=help`` 可列出所有命名的原色。
    （ ``--vo=gpu-next`` 独占）
...
``--video-sync=<audio|...>``
    ...
    :display-adrop:           丢弃或重复音频数据以补偿不同步的视频。如果真实的显示器刷新率与报告的或强制的刷新率相差太大，这种模式将引起严重的音频瑕疵。这个模式作用于整全部音频帧，而不是单个采样。
    :display-desync:          同步视频到显示，并让音频自行播放。视频与音频可能会出现任意程度的不同步，直到通过手动快进/快退进行调整为止。
...
``--external-files=<file-list>``
    ...
    与 ``--sub-files`` 和 ``--audio-files`` 不同，它包括所有轨道，不会导致默认流选择在“正确的”文件上。这使得它稍微不那么具有侵入性。
