...
``--hwdec=<api>``
    ...
    硬件解码在默认情况下是不启用的，因为它通常是一个额外的错误来源。只有当你的CPU太慢而无法解码特定视频的情况下，才值得使用。

    .. note::

        使用 ``Ctrl+h`` 快捷键来在运行时切换硬解码。它在 ``auto`` 和 ``no`` 之间切换这个选项。

        不鼓励通过把它放入设置文件来始终启用硬解码。如果你使用Ubuntu软件包，删除 ``/etc/mpv/mpv.conf`` ，因为该软件包尝试通过设置 ``hwdec=vaapi`` 来默认启用HW解码（这不是很理想，甚至可能导致使用次优的wrappers）。或者至少把它改为 ``hwdec=auto-safe``

    如果你想启用硬件解码，请使用其中一种的“自动”模式。明确的选择某个模式主要是为了测试和调试。如果想在更新后继续工作或其它，在设置文件中明确某个选择是个坏主意。
    ...
    .. admonition:: 选择哪种方式？
        ...
        - 如果不确定，但希望硬件解码在默认情况下被启用，可以在 ``mpv.conf`` 中加入 ``hwdec=auto-safe`` ，并认识到这种使用情况不“真正”受支持，可能会导致问题。
    ...
    因为这些模式将解码后的视频复制回到系统RAM，它们的效率往往不如直接模式，而且可能对软解没有太大帮助。
    ...
    .. admonition:: 硬件解码的质量降级
        ...
        理论上，硬解不会降低视频质量（至少对于h264和HEVC编码是这样）。然而，由于视频输出API的限制，以及实际硬件解码器的错误，可能会有一些损失，甚至是非常不正确的结果。
        ...
        ``vaapi`` 和 ``d3d11va`` 是安全的。启用去隔行扫描（或仅仅是它们各自的后处理滤镜）可能至少会通过将输出转换为8位格式而降低色彩质量。
        ...
        所有其它的方法，特别是copy-back的方式（如 ``dxva2-copy`` 等）应该是安全的，尽管它们仍然可能导致随机的解码问题。至少，它们不应该影响图像的色彩。

        特别的是， ``auto-copy`` 将只选择“安全”模式（尽管可能比其它方式慢），但仍然不能保证所选择的硬件解码器能真正正确的工作。

        一般而言，强烈建议避免使用硬解，除非 **绝对** 必要，例如，如果CPU不足以解码文件的问题。如果你遇到任何奇怪的解码问题，帧故障或变色，并且你打开了 ``--hwdec`` ，应该首先尝试禁用它。
...
``--stop-screensaver``, ``--no-stop-screensaver``
    在启动时关闭屏幕保护程序（或屏幕blanker和类似机制），在退出时再次打开（默认： yes）。当播放器暂停时，屏保总是被重新启用。
...
``--gamma=<-100-100>``
    调整视频信号的伽玛（默认值： 0）。不是所有的视频输出驱动都支持。
