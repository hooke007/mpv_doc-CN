...
``--drag-and-drop=<no|auto|replace|append>``
    (X11和Wayland和Windows独占)
    控制拖放在支持该功能的平台上的默认行为。 ``auto`` 将服从底层操作系统/平台给mpv的指示。通常情况下，在拖放过程中按住shift会将项目追加到播放列表中，否则新文件将完全取代旧文件。 ``replace`` 和 ``append`` 总是分别强制替换和追加到播放列表中。 ``no`` 禁用所有拖拽和拖放行为。
...
``--screenshot-avif-opts=key1=value1,key2=value2,...``
    ...
    .. admonition:: 示例
        ...
        "``--screenshot-avif-opts=crf=32,aq-mode=complexity``"
            将 crf 设置为 32，将量化（aq-mode）设置为基于复杂度。
