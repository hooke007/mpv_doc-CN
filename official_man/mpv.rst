...
参见 ``--input-test`` 获取按键的交互绑定详情，以及内置脚本 `统计数据`_ 以获取按键绑定列表（包括输出到终端的内容）。默认情况下， ? 键可切换显示该列表。
...
w, W
    减少/增加平移和扫描范围。目前 ``e`` 键与 ``W`` 键的作用相同但不赞成使用。更多参见 ``--panscan``
...
u
    在应用仅 ``--sub-ass-*`` 相关项的覆盖（默认值）到SSA/ASS字幕和几乎完全覆盖它们的普通字幕样式之间切换。详见 ``--sub-ass-override``

V
    切换视频数据用于ASS渲染的模式。详见 ``--sub-ass-use-video-data``
...
Ctrl+v
    将剪贴板中的文件或 URL 添加到播放列表中。如果当前未播放任何内容，则会立即播放。仅适用于支持 ``clipboard`` 属性的平台。

Ctrl+r
    重新加载当前文件，同时保留时间位置和所有已更改的选项。清空所有网络缓存。

i, I
    显示/切换一个显示关于当前播放文件的统计数据的覆盖层，比如编解码器、帧率、丢帧数等。详见 `统计数据`_

?
    切换显示当前活动的按键绑定的叠加层。详见 `统计数据`_
...
Alt+0 （macOS用 Command+0 ）
    调整视频窗口到原来的一半

Alt+1 （macOS用 Command+1 ）
    调整视频窗口到原来的大小

Alt+2 （macOS用 Command+2 ）
    调整视频窗口到原来的两倍

Command+f （macOS独占）
    切换全屏（另见 ``--fs`` ）

（以下按键绑定可在控制台中打开一个菜单，通过输入所需项目部分的内容、单击所需项目或使用按键绑定导航，从项目列表中进行选择： ``Down`` 和 ``Ctrl+n`` 向下滚动， ``Up`` 和 ``Ctrl+p`` 向上滚动， ``PGDWN`` 和 ``Ctrl+f`` 向下滚动一页， ``PGUP`` 和 ``Ctrl+b`` 向上滚动一页。）
...
g-h
    从观看历史里选择一个文件。需要 ``--save-watch-history``

g-w
    从稍后观看里选择一个文件以恢复播放进度（参见 `恢复播放`_ ）。需要 ``--write-filename-in-watch-later-config``
...
对于以 ``--`` 开头的选项，mpv 期望您在 ``=`` 之后传入选项值。例如，``mpv --log-file f.txt`` 将会失败，因为 ``--log-file`` 需要一个选项值，而 ``f.txt`` 会被视为一个待播放的普通文件（就像 ``mpv f.txt`` 那样）。
...
Shell实际上可能从传递到命令行的字符串中剥离一些引号，所以这个例子对字符串进行了两次引用，确保mpv接收到引号 ``"`` 。
 ...
 ``[...]`` 形式的引用包括了在 ``[`` 和 ``]`` 之间的一切东西。它对那些不解析参数中间的这些字符的shells很有用（比如bash）。这些引号是对称的： ``[`` 和 ``]`` 嵌套，引号终止于字符串中没有匹配 ``[`` 的最后一个 ``]`` 。（例如， ``[a[b]c]`` 的结果是 ``a[b]c`` 。）
...
传递给 mpv 的 URL 应进行百分比编码，才能确保其正常工作。虽然系统内置了一些启发式算法试图自动进行编码，但这些算法并非万无一失。例如，若要播放一个实际命名为 ``foo%20.mp4`` 的文件，直接使用未进行百分比编码的 ``http://localhost/foo%20.mp4`` 将无法播放。将其百分比编码为 ``http://localhost/foo%2520.mp4`` 则能正常播放。
...
对于传递给 mpv 子选项（具有多个 `:` 和 `,` 分隔值的选项）的路径，由于需要转义特殊字符，情况变得更加复杂。为了解决这个问题，可以用“固定长度 ”语法来封装路径，例如 ``%n%string_of_length_n`` （见上文）。

当使用libmpv API时，你应该严格避免使用 ``mpv_command_string`` 来调用 ``loadfile`` 命令，而应该选择例如 ``mpv_command`` 来避免文件名转义的需要。

使用脚本 API 时也是如此，应避免使用 ``mp.command`` ，而应使用 ``mp.commandv`` 和 ``mp.command_native`` 等“独立参数”API。

一些 mpv 选项会对以 ``~`` 开头的路径解析特殊含义，从而方便动态查找特殊目录，例如指向当前用户的home目录或 mpv 设置目录。

使用特殊的 ``~`` 前缀时，特殊路径前缀后必须始终有一个尾缀 ``/`` 。换句话说， ``~`` 不起作用，但 ``~/`` 有效。

目前可识别以下特殊路径/关键字：

.. warning::

    请注意，如果使用了 ``--no-config`` ，所有基于“设置目录”的路径（ ``~~/`` 、``~~home/`` 和 ``~~global/`` ）都将是空字符串。

    这意味着 ``~~home/`` 将展开为空字符串，子路径（如 ``~~home/foo/bar"`` ）将展开为相对路径 ( ``foo/bar`` )，这可能与你的预期不同。

    此外，任何在设置目录中搜索的命令都将找不到任何东西，因为没有任何目录可以搜索。

    请确保你的脚本可以处理这些 "no config" 的情况。

================ ===============================================================
名称             意义
================ ===============================================================
``~/``           当前用户的home目录（相当于终端环境中的 ``~/`` 和 ``$HOME/`` ）
...
-clr          清除选项（移除所有项目）
-del          删除1个或多个（如果存在）项目（与 -set 的语法相同）
...
这对一些选项是一个非常复杂的选项类型，例如 ``--af`` 和 ``--vf`` 选项。它们通常需要复杂的转义。详见 `视频滤镜`_ 。
...
后缀          含义
============= ===============================================
-set          设置一个对象列表（使用 ``,`` 作为分隔符）
-append       追加单个对象
-add          追加1个或多个对象（与 -set 的语法相同）
-pre          前置1个或多个对象（与 -set 的语法相同）
-clr          清除选项（移除所有对象）
-remove       删除1个或多个（如果存在）项目（与 -set 的语法相同）
-toggle       追加一个对象，如果它已存在列表中，则移除
...
一些操作像 ``-add`` 或 ``-pre`` 如指定了多个项目，但请注意，可能需要转义参数。 ``-append`` 只接受一个未转义的项目（因此分隔符 ``,`` 不会被解析，而是作为值的一部分传递）。
...
这与命令行选项的处理方式相同。配置项可以使用双引号（ ``"`` , ``'`` ）引用，也可以使用之前提到的固定长度语法（ ``%n%`` ）引用。这就好像将引用字符串的确切内容作为命令行选项传递一样。在这个级别上，不解释C风格的转义字符，尽管某些选项会手动执行这个操作（这是一种混乱，可能会在某个时候改变）。这里不涉及shell，因此只需要引用选项值以转义值的任意位置的 ``#`` ，或 ``"`` 或 ``'`` 或 ``%`` ，以及前导符与尾随的空格。
...
从其它程序或脚本中使用mpv，有三种选择：
    ...
    1. 把它当作UNIX的进程调用。如果你这样做的话， *不要解析终端的输出* 。终端输出适用于人，并可能随时发生变化。另外，终端的行为本身也可能随时改变。兼容性无法得到保证。
       ...
       如果你传递了 ``--terminal=no`` ，你的代码也应该起效。不要尝试通过向mpv的stdin发送终端控制代码来模拟用户输入。如果你需要交互式控制，推荐使用  ``--input-ipc-server`` 或 ``--input-ipc-client`` 。这可以让你通过unix域套接字（或Windows的命名管道）访问 `JSON IPC`_ 

       根据你的想做的事情，传递 ``--no-config`` 或 ``--config-dir`` 可能是一个好方案，以避免与用于CLI播放的常规mpv用户设置发生冲突。

       使用 ``--input-ipc-server`` 或 ``--input-ipc-client`` 也适用于像远程控制等的目的（然而，IPC协议自身并不 "安全"，它也不打算变得安全）。
...
另一个常见的问题是MJPEG流。这些流没有正确的帧率信号。使用 ``--untimed`` 或 ``--correct-pts=no --container-fps-override=60`` 可能有所帮助。
...
始终用绑定到 ``quit-watch-later`` 的按键退出和使用 ``--save-position-on-quit`` 的区别是，即使mpv是用绑定键以外的方法关闭的，后者都会保存播放位置，例如点击窗口标题栏上的关闭按钮。但是，如果 mpv 突然终止来不及保存，那么播放位置就不会被记住。例如，在未关闭 mpv 的情况下关闭系统。

mpv也保存除播放位置以外的其它选项，当它们在播放开始后已被修改，例如音量和已选的音轨/字幕轨，并在下次播放文件时恢复它们的值。可以用 ``--watch-later-options`` 选项进行设置哪些选项会被保存。
...
``mpv://...``
    ...
    mpv协议。这用于从URL处理程序启动mpv。协议被剥离，其余部分作为正常的公开参数传递给播放器。只有安全的网络协议才允许以这种方式打开。
...
``http://...``, ``https://``, ...
    ...
    ``data:`` 受支持，但需要采用 ``data://`` 的格式。这样做是为了避免与文件名产生歧义。你也可以用 ``lavf://`` 或 ``ffmpeg://`` 作为前缀。
...
``mf://[@listfile|filemask|glob|printf-format]`` ``--mf-...``
    ...
    如果 URL 路径以 ``@`` 开头，它将被解释为包含一系列图像路径的文件路径，这些图像路径由换行符分隔。如果 URL 路径包含 ``,`` ，则会被解析为由 ``,`` 分隔的图像路径列表。如果 URL 路径不包含 ``%`` ，且在 POSIX 平台上，则被解析为 glob，如果未指定 ``*`` ，则自动添加。否则，将解析 printf 序列 ``%[.][NUM]d`` （其中 ``NUM`` 为一位、两位或三位小数）和 ``%%`` 。例如， ``mf://image-%d.jpg`` 可以播放 `image-1.jpg`` 、 ``image-2.jpg`` 和 ``image-10.jpg`` 等文件，前提是文件之间没有大的间隔。

``cdda://[device]`` ``--cdda-device=PATH``

    播放CD。您可以使用 ``--start`` 和 ``--end`` 选项并指定章节来选择要播放的特定曲目范围。通过章节导航（默认按键绑定中的 ``PGUP`` 和 ``PGDOWN`` ）
...
``lavf://...``
    ...
    访问任何FFmpeg的libavformat协议。基本上，这把 ``//`` 后面的字符串直接传递给了libavformat。
...
``archive://[ARCHIVE PATH]|[FILE PATH IN ARCHIVE]``
    ...
    打开归档文件中指定路径下的文件。需要启用 libarchive 功能。归档路径中的所有 ``%`` 和 ``|`` 字符必须进行 URL 转义。文件路径不应进行 URL 转义。
    ...
    .. admonition:: 示例
        ...
        这将播放存档文件 ``file.zip`` 中的 ``video.mkv`` 。
...
.. warning::
    ...
    目前，你可以在设置文件中以正常的方式扩展 ``pseudo-gui`` profile。这已经过时，将在未来的 mpv 版本中移除。

    作为替代方案，也可以使用附带条件的自动配置预设：
...
``MPV_LEAK_REPORT``
    如果设为 ``1`` ，启用内部talloc泄漏报告。如果设为其它值，禁用泄漏报告。
...
根据操作系统的不同，mpv默认会选择不同的磁盘设备。这适用于所有光盘播放（CDDA, DVD 和 BD）。
...
请注意，本节假定使用 Linux/BSD。在其它平台上，路径可能有所不同。有关Windows的具体内容，参见 `WINDOWS的文件`_ 部分。
...
在 macOS 上，稍后观看目录位于 ``~/.config/mpv/watch_later/`` ，缓存目录设置为 ``~/Library/Caches/io.mpv/`` 。这些目录不能被环境变量覆盖。其它的内容与 `文件`_ 一样。
