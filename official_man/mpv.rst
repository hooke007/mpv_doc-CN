...
（以下按键绑定可在控制台中打开一个选择器，通过键入所需项目的部分内容和/或通过按键绑定导航，从项目列表中进行选择：  ``Down`` 和 ``Ctrl+n`` 向下滚动， ``Up`` 和 ``Ctrl+p`` 向上滚动， ``PGDWN`` 和 ``Ctrl+f`` 向下滚动一页， ``PGUP`` 和 ``Ctrl+b`` 向上滚动一页。）
...
从其它程序或脚本中使用mpv，有三种选择：
    ...
    1. 把它当作UNIX的进程调用。如果你这样做的话， *不要解析终端的输出* 。终端输出适用于人，并可能随时发生变化。另外，终端的行为本身也可能随时改变。兼容性无法得到保证。
       ...
       如果你传递了 ``--terminal=no`` ，你的代码也应该起效。不要尝试通过向mpv的stdin发送终端控制代码来模拟用户输入。如果你需要交互式控制，推荐使用  ``--input-ipc-server`` 。这可以让你通过unix域套接字（或Windows的命名管道）访问 `JSON IPC`_ 
